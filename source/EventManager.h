...
class BLOCOCORE_API IEventData;
typedef shared_ptr<IEventData> BLOCOCORE_API IEventDataPtr;

class BLOCOCORE_API IEventData : public ILuaable
{
public:
...
class BLOCOCORE_API BaseEventData : public IEventData
{
public:
...
template < typename T >
struct BLOCOCORE_API EvtData : public BaseEventData
{
private:
...
typedef EvtData<int> BLOCOCORE_API EvtData_Int;

// struct EvtData_ScriptEvtData					- Chapter 11, page 328
//This type of event data is created by script-defined events.
struct BLOCOCORE_API EvtData_ScriptEvtData : public BaseEventData
{
public:
...
// imposed or assumed.
class BLOCOCORE_API IEventListener
{
public:
...
typedef shared_ptr<IEventListener> BLOCOCORE_API   EventListenerPtr;
//typedef concurrent_queue<IEventDataPtr> ThreadSafeEventQueue;

class BLOCOCORE_API IEventManager
{
public:
...
private:
	...
	friend bool BLOCOCORE_API safeAddListener( EventListenerPtr const & inHandler,
		EventType const & inType );

	friend bool BLOCOCORE_API safeDelListener( EventListenerPtr const & inHandler,
		EventType const & inType );

	friend bool BLOCOCORE_API safeTriggerEvent( IEventData const & inEvent );

	friend bool BLOCOCORE_API safeQueEvent( IEventDataPtr const & inEvent );
	friend bool BLOCOCORE_API threadSafeQueEvent( IEventDataPtr const & inEvent );

	friend bool BLOCOCORE_API safeAbortEvent( EventType const & inType,
		bool allOfType = false );

	friend bool BLOCOCORE_API safeTickEventManager( unsigned long maxMillis =
		IEventManager::kINFINITE );

	friend bool BLOCOCORE_API safeValidateEventType( EventType const & inType );

	friend void BLOCOCORE_API safeSetDebugView(DebugView* pDebugView);
	friend void BLOCOCORE_API safeRemoveDebugView();
};


extern IEventManager BLOCOCORE_API* g_pEventMgr;
...
bool BLOCOCORE_API  threadSafeQueEvent( IEventDataPtr const & inEvent );
bool BLOCOCORE_API  safeAbortEvent( EventType const & inType, bool allOfType /* = false */ );
bool BLOCOCORE_API  safeTickEventManager( unsigned long maxMillis /* = IEventManager::kINFINITE */ );
